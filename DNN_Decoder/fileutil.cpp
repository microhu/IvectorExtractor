...
void freadOrDie (void * ptr, size_t size, size_t count, FILE * f)
{
    ...
    while (count > 0)
    {
        size_t chunkn = min (count, 15*1024*1024);
...
// 'input' must exist, 'target' may not exist
bool msra::files::fuptodate (const wstring & target, const wstring & input)
{
    FILETIME targettime;
    if (!getfiletime (target, targettime)) return false;    // target missing: need to update
    FILETIME inputtime;
    if (!getfiletime (input, inputtime)) return false;      // input missing: pretend to be out of date as to force caller to fail
